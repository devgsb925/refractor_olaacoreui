...
export class DesktopComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    if (this.viewModel.getAllImageList().length > 0) {
      if (this.viewModel.getImageList(1).length > 0) {
        this.viewModel.bannerSelectId = this.viewModel
          .getImageList(1)
          .sort((a, b) => a.orderIndex - b.orderIndex)[0].id;
      } else if (this.viewModel.getImageList(4).length > 0) {
        this.viewModel.bannerSelectId = this.viewModel
          .getImageList(4)
          .sort((a, b) => a.orderIndex - b.orderIndex)[0].id;
      } else if (this.viewModel.getImageList(5).length > 0) {
        this.viewModel.bannerSelectId = this.viewModel
          .getImageList(5)
          .sort((a, b) => a.orderIndex - b.orderIndex)[0].id;
      } else if (this.viewModel.getImageList(6).length > 0) {
        this.viewModel.bannerSelectId = this.viewModel
          .getImageList(6)
          .sort((a, b) => a.orderIndex - b.orderIndex)[0].id;
      }
  ...
  ngOnDestroy(): void {
    this.subsription.forEach((s) => s.unsubscribe());
  }
  ...
  getSelectCategory(): ICategory {
    return this.viewModel
      .getCategories()
      .find((cat) => cat.id === this.viewModel.categorySelectId);
  }
  getPreviewCategory(): string {
    const pendingCat = this.viewModel
      .getAllPendingList()
      .find((pend) => pend.refCatId == this.viewModel.categorySelectId);
    if (pendingCat !== undefined) {
      return pendingCat.imgSource;
    } else if (
      this.viewModel
        .getCategories()
        .find((cat) => cat.id === this.viewModel.categorySelectId) !== undefined
    ) {
      return this.viewModel
        .getCategories()
        .find((cat) => cat.id === this.viewModel.categorySelectId).bannerUrl;
    }
    return '';
  }
  ...
  getCategoryLink(): string {
    ...
    if (pendingCat !== undefined) {
      ...
      return this.viewModel
        .getCategories()
        .find((cat) => cat.id === this.viewModel.categorySelectId).linkUrl;
    }
    return '';
  }
  //#endregion
